# 创建脚本文件
cat > .github/scripts/build-ipk-packages.sh << 'SCRIPT_EOF'
#!/usr/bin/env bash
...
# 2. 设置可执行权限
chmod +x .github/scripts/build-ipk-packages.sh

# 3. 验证文件已创建
ls -la .github/scripts/build-ipk-packages.sh

# 4. 提交并推送
git add .github/scripts/build-ipk-packages.sh
git add .github/workflows/build-apk-packages.yml
git commit -m "添加 IPK 编译脚本 + 更新 workflow"
git push
